...
// Exported so the server's asset/tree walkers exclude the same noise
// directories. meta.excludes persists only the *user's* extra excludes, so
// consumers that walk the vault need this list too.
...
// The leading (?<!!) skips embeds: ![[image.png]] transcludes a file, it does not
// link to a note. Without it every embedded image, clip, and PDF became a link to
// a note that does not exist, and so a phantom "unwritten note" node — 142 of the
// 246-note demo vault's 563 phantoms were really just its attachments. The server
// resolves those same embeds as assets (/api/file), so they are already handled;
// they simply are not graph edges. MD_LINK carries the same guard for ![](...).
const WIKI_LINK = /(?<!!)\[\[([^\]|#\n]+)(?:#[^\]|\n]*)?(?:\|[^\]\n]*)?\]\]/g;
...
// v5: ![[embeds]] no longer parsed as wiki links, so cached links arrays from
//     v4 and earlier still carry attachment targets and must be rebuilt
